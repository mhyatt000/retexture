...
class DataFrameHeatMap:
    ...
    def process_data(self):
        ...
        self.df['viewpoint'] = self.df['path'].apply(lambda x: x.split("/")[-1].split("_")[-1].split(".")[0])
        self.df['model'] = self.df['path'].apply(lambda x: x.split("/")[-1].split("_")[0])
        self.df['texture'] = self.df['path'].apply(lambda x: x.split("/")[-1].split("_")[1])

        out_columns = [f"out{i}" for i in range(1000)]  # Adjust range if necessary

        for  (model,tex), sf in self.df.groupby(['model','texture']):
            print(model,tex)

            for view , ssf in sf.groupby('viewpoint'):
                print(view)

                outs = ssf[out_columns].sum().tolist()
                plt.bar([i for i in range(len(outs))], outs)
                plt.title(f'{model} {tex} {view}')
                plt.ylim(0,1)
                plt.show()

            # sf['top5'] = sf[out_columns].apply(lambda x: np.argsort(x)[-5:])
            quit()
        
        print(self.df['viewpoint'].unique())
        print(self.df['model'].unique())
        print(self.df['texture'].unique())
        quit()
